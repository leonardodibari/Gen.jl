...
function MCMC_Metropolis(nsteps::Int, seq::Array{Int,1}, L::Int, h::Array{Float64,1}, J::Array{Float64,2}, β::Float64, each_step = each_step)
...
function Multiple_MCMC_Metropolis(nsamples::Int, nsteps::Int, seq::Array{Int,1}, L::Int, h::Array{Float64,1}, J::Array{Float64,2}, β::Float64; RandomInitial::Bool=false, each_step = 10)
    ...
    step_msa  = [zeros(Int64,(nsamples,L)) for i in 1:((nsteps/each_step))]
    ...
    for n in 1:nsamples
        for i in 1:L
            step_msa[1][n,i] = evolving_MSA[n,i]
        end
    end
    
    println(mean(H_distance(seq, step_msa[1])))

    count =  0
    for t in 1:nsteps
        @tasks for s in 1:nsamples
            evolving_MSA[s,:] .= MCMC_Metropolis_onestepnoene(evolving_MSA[s,:], L, h, J, β)
        end
        ...
        if t%each_step == 0
            ...
            for i in 1:L
                for n in 1:nsamples
                    step_msa[count][n,i] = evolving_MSA[n,i]
                end
            end
        end    
...
function compute_empirical_freqs(Z::Array{Int64,2}, q::Int) 
    ...
    parameters:
    - Z: MSA in format Number_of_configurations x Size_of_system 
    - q: length of the alphabet (2)

    output:
    - f: matrix q x Size_of_system with the frequence of each spin value (+1,-1) at each site
    """
...
function myentropy(Z::Array{Int64,2}, q::Int)
    ...
    parameters:
    - Z: MSA in format Size_of_system x Number_of_configurations
    - q: length of the alphabet (2)

    output:
    - entr: vector of length Size_of_system with the entropy of each site
    """
...
function myCDE(conf::Array{Int,1}, h::Array{Float64,1}, J::Array{Float64,2}, β::Float64)

    """
    Function to compute the context-dependent entropy of each site from the model.

    parameters:
    - conf: configuration giving the context in which we compute the entropy
    - h: the local fields of the model used to compute the entropy
    - J: the couplings of the model used to compute the entropy
    - β: inverse temperature used to compute the probability

    output:
    - entr: vector of length Size_of_system with the context-dependent entropy of each site
    """
...
function myCDEsingle(site::Int, conf::Array{Int,1}, h::Array{Float64,1}, J::Array{Float64,2}, q::Int, β::Float64)

    """
    Function to compute the context-dependent entropy of a single site from the model.
    ...
    L = length(conf)
    _s = 0.0
    f = cond_proba(conf,h[:,site],J[:,:,:,site],L,β)
    for a in 1:q
        # _s -= f[a] > 0 ? f[a] * log(f[a]) : 0.0  # check the base of the logarithm
        _s -= f[a] > 0 ? f[a] * log2(f[a]) : 0.0
    end
...
#= function Multiple_MCMC_Metropolis_EntropyEvolution(nstore::Int, nsamples::Int, nsteps::Int, seq::Array{Int,1}, L::Int, h::Array{Float64,1}, J::Array{Float64,2}, β::Float64; RandomInitial::Bool=false)

    """
    Function to perform many MonteCarlo trajectories with the Metropolis setting.

    parameters:
    - nstore: the number of stored values of entropy
    - nsamples: number of samples to be computed
    - nsteps: the number of steps to be performed
    - seq: the initial configuration
    - L: the size of the system
    ...
    entr_vec[:,1] = myentropy(evolving_MSA,2)

    for n in 1:nsteps 
        for s in 1:nsamples
            evolving_MSA[s,:] .= MCMC_Metropolis_onestepnoene(evolving_MSA[s,:], L, h, J, β)
        end
        if n%steps_storing == 0 && n<nsteps 
            entr_vec[:,n÷steps_storing+1] = myentropy(evolving_MSA,2)
        end
    end
